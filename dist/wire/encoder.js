...
function collectNamesFromType(t, c) {
    switch (t.kind) {
        case 'Int':
        case 'Float':
        case 'Str':
        ...
        case 'Fn':
            for (const p of t.params)
                collectNamesFromType(p, c);
            collectNamesFromType(t.ret, c);
            break;
        case 'User':
            c.record(t.name);
            for (const a of t.args)
                collectNamesFromType(a, c);
            break;
        default:
            assertNever(t);
    }
}
function collectNamesFromPattern(p, c) {
    switch (p.kind) {
        case 'Wildcard':
        case 'Literal':
            break;
        ...
        case 'Constructor':
            ...
            for (const f of p.fields)
                collectNamesFromPattern(f, c);
            break;
        default:
            assertNever(p);
    }
}
function collectNamesFromNode(node, c) {
    ...
    switch (node.kind) {
        ...
        case 'Case':
            ...
            for (const arm of node.arms) {
                ...
                if (arm.guard !== undefined)
                    collectNamesFromNode(arm.guard, c);
        ...
        case 'Accessor':
            ...
            break;
        case 'ModuleRef':
            for (const p of node.modulePath)
                c.record(p);
            break;
        case 'ForeignRef':
        ...
        case 'Handle':
            ...
            if (node.returnClause !== undefined)
                collectNamesFromNode(node.returnClause, c);
            break;
        case 'Resume':
            collectNamesFromNode(node.value, c);
            break;
...
function collectNames(module) {
    const c = new NameCollector();
    for (const imp of module.imports) {
        for (const p of imp.modulePath)
            c.record(p);
        collectNamesFromType(imp.type, c);
    }
    for (const d of module.data)
        collectNamesFromNode(d, c);
    for (const decl of module.decls)
        collectNamesFromNode(decl, c);
    return c.result();
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function collectTypesFromType(t, out) {
    ...
    switch (t.kind) {
        ...
        case 'TypeVar':
            break;
        case 'List':
            collectTypesFromType(t.elem, out);
            break;
        case 'Option':
            collectTypesFromType(t.inner, out);
            break;
        case 'Map':
        ...
        case 'Fn':
            for (const p of t.params)
                collectTypesFromType(p, out);
            collectTypesFromType(t.ret, out);
            break;
        case 'User':
            for (const a of t.args)
                collectTypesFromType(a, out);
            break;
        default:
            assertNever(t);
    }
}
function collectTypesFromNode(node, out) {
    ...
    switch (node.kind) {
        ...
        case 'App':
            ...
            for (const a of node.args)
                collectTypesFromNode(a, out);
            break;
        case 'Abs':
            for (const p of node.params)
                collectTypesFromType(p.type, out);
        ...
        case 'Case':
            ...
            for (const arm of node.arms) {
                if (arm.guard !== undefined)
                    collectTypesFromNode(arm.guard, out);
        ...
        case 'Accessor':
            ...
            break;
        case 'ForeignRef':
            for (const p of node.sig.params)
                collectTypesFromType(p, out);
        ...
        case 'OopClass':
            for (const f of node.fields)
                collectTypesFromType(f.type, out);
            for (const m of node.methods) {
                for (const p of m.params)
                    collectTypesFromType(p.type, out);
                collectTypesFromType(m.retType, out);
                if (m.body !== undefined)
                    collectTypesFromNode(m.body, out);
            }
            break;
        case 'OopInterface':
            for (const mem of node.members)
                collectTypesFromType(mem.type, out);
            break;
        case 'OopNew':
            for (const a of node.args)
                collectTypesFromNode(a, out);
        ...
        case 'AsyncBlock':
            ...
            break;
        case 'Await':
            collectTypesFromNode(node.expr, out);
            break;
        case 'AdtDecl':
            for (const v of node.variants) {
                for (const f of v.fields)
        ...
        case 'AdtMatch':
            ...
            for (const arm of node.arms) {
                for (const b of arm.bindings)
                    collectTypesFromType(b.type, out);
                collectTypesFromNode(arm.body, out);
            }
            break;
        case 'EffectDecl':
        ...
        case 'Handle':
            ...
            if (node.returnClause !== undefined)
                collectTypesFromNode(node.returnClause, out);
            break;
        case 'Resume':
            collectTypesFromNode(node.value, out);
            break;
...
function collectTypes(module) {
    const out = new Map();
    for (const imp of module.imports)
        collectTypesFromType(imp.type, out);
    for (const d of module.data)
        collectTypesFromNode(d, out);
    for (const decl of module.decls)
        collectTypesFromNode(decl, out);
    return out;
}
// ---------------------------------------------------------------------------
...
function tokenCost(len) {
    return Math.ceil(len / 4);
}
function shouldPool(len, count) {
 ...
 */
function buildSymbolPool(names) {
 ...
 */
function buildTypePool(typeExprs) {
...
function encodeDataLines(data, ctx) {
    if (data.length === 0)
        return '';
    const parts = data.map(adt => {
        ...
        const variants = adt.variants.map(v => {
            ...
            const fields = v.fields.map(f => `${encodeName(f.name, ctx.sym)}:${encodeType(f.type, ctx)}`).join(',');
            return `${tag}{${fields}}`;
        });
...
function encodeDeclLines(decls, ctx) {
    if (decls.length === 0)
        return '';
...
function encodeNode(node, ctx) {
    switch (node.kind) {
        ...
        case 'OopInterface': {
            const mems = node.members.map(m => `${encodeName(m.name, ctx.sym)}:${encodeType(m.type, ctx)}`).join(',');
