...
function ind(depth, size) {
    return ' '.repeat(depth * size);
}
function printLiteral(v) {
    switch (v.kind) {
...
function printCaseArm(arm, depth, size) {
...
function printAdtArm(arm, depth, size) {
...
function printTopLevelDecl(node, size, out) {
    if (node.kind === 'Let') {
...
export function prettyPrintType(type) {
    switch (type.kind) {
        case 'Int': return 'Int';
...
export function prettyPrintNode(node, depth = 0, opts) {
...
export function prettyPrintModule(mod, opts) {
    const size = opts?.indentSize ?? 2;
    const sections = [];
    const header = [`module ${mod.module}  "${mod.version}"`];
