...
import { countTokens, type TokenizerName } from './tokenizer.js';
...
function parseArgs(args: string[]): ParsedArgs | { error: string } {
  ...
  let tokenizer: TokenizerName = 'cl100k';
  let json = false;
  let baselinePath: string | null = null;
  let saveBaselinePath: string | null = null;
  ...
  while (i < args.length) {
    ...
    if (arg === '--tokenizer') {
      ...
      i++;
      const val = args[i];
      if (val === undefined) return { error: '--baseline requires a path argument' };
      baselinePath = val;
    } else if (arg === '--save-baseline') {
      i++;
      const val = args[i];
      if (val === undefined) return { error: '--save-baseline requires a path argument' };
      saveBaselinePath = val;
    } else if (arg !== undefined && !arg.startsWith('--')) {
      if (filePath !== null) return { error: 'unexpected extra positional argument' };
      filePath = arg;
  ...
  if (filePath === null) return { error: 'missing required positional argument: <file>' };
...
function formatHuman(report: TokenReport): string {
  const lines = [
    `file:      ${report.file}`,
 ...
 */
export async function runTokens(args: string[]): Promise<RunResult> {
  ...
  if (baselinePath !== null) {
    ...
    if (comparison.exceeded) {
      process.stdout.write(
        `CI FAILURE: wire token count increased by ${comparison.wireChangePct.toFixed(2)}% (threshold: +2.00%)\n`,
      );
      exitCode = 1;
    }
  }
