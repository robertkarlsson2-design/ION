...
import type { BindError, BindProgramResult } from '../../src/binder/index.js';
...
function publicNames(result: BindProgramResult): Set<string> {
  return new Set(allSymbols(result).filter(s => s.pub).map(s => s.name));
}

function resolutionValues(result: BindProgramResult) {
  return Array.from(result.modules[0]?.symbolTable.references.values() ?? []);
}
...
describe('bindModule — fn declaration', () => {
  it('registers pub fn and its param, resolves param reference in body', () => {
    ...
    expect(resolutionValues(result)).toContain(xSym?.id);
  });
...
describe('bindModule — LetExpr sequential binding', () => {
  it('resolves inner LetExpr referencing the outer binding', () => {
    ...
    const refValues = resolutionValues(result);
...
describe('bindModule — forward reference', () => {
  it('resolves forward references via two-pass hoisting', () => {
    ...
    expect(resolutionValues(result)).toContain(bSym?.id);
  });
});
